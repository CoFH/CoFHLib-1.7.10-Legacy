		...
		for (int l = clusterSize; l-- > 0;) {
			...
			if ((!seeSky || world.canSeeSky(new BlockPos(x, y, z))) && canGenerateInBlock(world, x, y - 1, z, onBlock)
				...
				do {
					...
					if (!checkStay || (block.block instanceof BlockBush && ((BlockBush) block.block).canBlockStay(world, pos, state))) {
						r |= world.setBlockState(pos, block.block.getStateFromMeta(block.metadata), 2);
					} else {
						break;
					}
					++y;
