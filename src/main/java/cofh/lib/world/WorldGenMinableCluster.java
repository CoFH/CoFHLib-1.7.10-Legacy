...
public class WorldGenMinableCluster extends WorldGenerator {
	...
	@Override
	public boolean generate(World world, Random rand, BlockPos startPos) {

		//TODO clean up and replace with BlockPos calls
		int x = startPos.getX();
		int y = startPos.getY();
		int z = startPos.getZ();
	...
	public static boolean canGenerateInBlock(World world, int x, int y, int z, WeightedRandomBlock[] mat) {
		...
		for (int j = 0, e = mat.length; j < e; ++j) {
			...
			if ((-1 == genBlock.metadata || genBlock.metadata == block.getMetaFromState(state))
					&& (block.isReplaceableOreGen(state, world, pos, BlockMatcher.forBlock(genBlock.block)) || block.isAssociatedBlock(genBlock.block))) {
				return true;
			}
		}
		return false;
	...
	public static boolean generateBlock(World world, int x, int y, int z, List<WeightedRandomBlock> o) {
		...
		return world.setBlockState(new BlockPos(x, y, z), ore.block.getStateFromMeta(ore.metadata), 2);
	}
