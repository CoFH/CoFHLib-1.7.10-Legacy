...
public class WorldGenMassiveTree extends WorldGenerator {
				...
				if (distSq > maxDistSq) {
					...
					for (t = -1; t <= 1; t += 2) {
						...
						if (safeGrowth ? canGenerateInBlock(worldObj, x, y, z, genBlock)
								&& (!treeChecks || (worldObj.isAirBlock(pos) || block.isLeaves(state, worldObj, pos) || block.canBeReplacedByLeaves(state,
										worldObj, pos))) : block.getBlockHardness(state, worldObj, pos) >= 0) {
		...
		for (int i = -trunkSize; i <= trunkSize; i++) {
			...
			for (int j = -trunkSize; j <= trunkSize; j++) {
				if ((j * j + i * i) * 4 < trunkSize * trunkSize * 5) {
					...
					if (smoothLogs) {
						BlockPos pos = new BlockPos(var6[0], var6[1], var6[2]);
						this.setBlockAndNotifyAdequately(worldObj, pos, worldObj.getBlockState(pos).getBlock().getStateFromMeta(12));
					}
					BlockPos pos = new BlockPos(var5[0], var5[1] - 1, var5[2]);
					IBlockState state = worldObj.getBlockState(pos);
					state.getBlock().onPlantGrow(state, worldObj, pos, new BlockPos(var1, var2, var4));
				}
			}
		}
	}
	...
	private int checkBlockLine(int[] par1, int[] par2) {
		...
		if (var3[var5] == 0) {
			...
			for (; var14 != var15; var14 += var8) {
				...
				if (safeGrowth ? canGenerateInBlock(worldObj, x, y, z, genBlock)
						&& (!treeChecks || !(worldObj.isAirBlock(pos) || var16.isReplaceable(worldObj, pos)
								|| var16.canBeReplacedByLeaves(state, worldObj, pos) || var16.isLeaves(state, worldObj, pos) || var16.isWood(worldObj, pos) || var16 instanceof BlockSapling))
						: var16.getBlockHardness(state, worldObj, pos) >= 0) {
					break;
				}
			}
	 ...
	 */
	//TODO setScale is gone, so this very likely needs some other way of handling
/*
	@Override
...
*/
	...
	public synchronized boolean generate(World world, Random par2Random, BlockPos pos) {
		...
		//TODO refactor the logic to use BlockPos
		int x = pos.getX();
		int y = pos.getY();
		int z = pos.getZ();
	...
	@Override
	public void setBlockAndNotifyAdequately(World world, BlockPos position, IBlockState state) {
		...
		if (!fastPlacement) {
			if (state != null) {
				super.setBlockAndNotifyAdequately(world, position, state);
			}
			return;
		}
		// ++blocksAdded;
		...
		if (chunk == null) {
			chunk = world.getChunkFromBlockCoords(new BlockPos(x, 1, z));
			chunkMap.put(pos, chunk);
		}

		//TODO is there a use for ExtendedBlockStorage with new BlockStates when it comes to tree gen? if so this needs some reimplementation
/*
		ExtendedBlockStorage[] storage = chunk.getBlockStorageArray();
